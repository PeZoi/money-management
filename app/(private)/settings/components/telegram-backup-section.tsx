...
import { useTelegramBackup } from '../hooks/use-telegram-backup';
import { cn } from '@/lib/utils';

type BackupState = ReturnType<typeof useTelegramBackup>;
...
// 2. Component trạng thái chưa kết nối
interface UnconnectedCardProps {
  onConnect: () => void;
  isGenerating: boolean;
}
function UnconnectedCard({ onConnect, isGenerating }: UnconnectedCardProps) {
  return (
...
// 3. Component thông tin đã kết nối
interface ConnectedStatusCardProps {
  state: BackupState;
}
function ConnectedStatusCard({ state }: ConnectedStatusCardProps) {
  const { connection, handleCheckConnection, isCheckingConnection, disconnect, isDisconnecting } = state;
...
// 4. Component sao lưu thủ công
interface ManualBackupCardProps {
  state: BackupState;
}
function ManualBackupCard({ state }: ManualBackupCardProps) {
  const { handleTriggerBackup, isBackingUp, showBackupDialog } = state;
...
// 5. Component sao lưu định kỳ
interface AutoBackupCardProps {
  state: BackupState;
}
function AutoBackupCard({ state }: AutoBackupCardProps) {
  const { connection, getScheduleDescription, updateConfig, isUpdatingConfig } = state;
...
// 6. Component Khôi phục dữ liệu bằng Drag & Drop
interface RestoreDataCardProps {
  state: BackupState;
}
function RestoreDataCard({ state }: RestoreDataCardProps) {
  const {
    handleDragOver,
    handleDragLeave,
    handleDrop,
...
// 7. Dialog Hướng dẫn liên kết Telegram
interface ConnectDialogProps {
  state: BackupState;
}
function ConnectDialog({ state }: ConnectDialogProps) {
  const { openConnectDialog, setOpenConnectDialog, connectLink, handleCheckConnection, isCheckingConnection } = state;
...
// 8. Dialog Trạng thái sao lưu thủ công
interface BackupProgressDialogProps {
  state: BackupState;
}
function BackupProgressDialog({ state }: BackupProgressDialogProps) {
  const { showBackupDialog, backupProgress } = state;
...
// 9. Dialog Trạng thái & Kết quả khôi phục dữ liệu
interface ImportProgressDialogProps {
  state: BackupState;
}
function ImportProgressDialog({ state }: ImportProgressDialogProps) {
  const { isImporting, setIsImporting, importResult, importProgress, importStep } = state;
...
export default function TelegramBackupSection() {
  ...
  return (
    <div className="grid gap-6">
      ...
      {/* 2. Trạng thái kết nối & Cài đặt */}
      {!state.connection.connected ? (
        <UnconnectedCard
          onConnect={state.handleOpenConnect}
          isGenerating={state.isGeneratingToken}
        />
      ) : (
        <>
          {/* Trạng thái đã liên kết */}
          <ConnectedStatusCard state={state} />

          {/* Các nút Trigger & Settings */}
          <div className="grid gap-6 sm:grid-cols-2 items-start">
            <ManualBackupCard state={state} />
            <AutoBackupCard state={state} />
          </div>
        </>
      )}

      {/* 3. Khôi phục dữ liệu Drag & Drop */}
      <RestoreDataCard state={state} />

      {/* 4. Các Dialogs quản lý trạng thái */}
      <ConnectDialog state={state} />
      <BackupProgressDialog state={state} />
